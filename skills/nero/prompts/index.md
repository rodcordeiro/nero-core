...
| `agents-md-references.txt` | Criar/atualizar `AGENTS.md` enxuto + `.agents/references/` por responsabilidade (api/front/mobile/integracoes). Apos classificar o dominio, ler e aplicar o guideline em `references/guidelines/` (`api`/`front`/`mobile`; `integracoes` herda api) sem copiar o texto para o app — so ponteiro + debito se o checkout divergir. Domain Skills: `references/domain-skills.md`. |
