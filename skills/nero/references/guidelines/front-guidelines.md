...
Fonte canônica na skill `$nero`: `references/guidelines/front-guidelines.md`.
...
Use the local framework conventions first. For Next.js, respect the existing `app/` or `pages/` router model. For Vite/SPA React, respect the current router, layout, state, and feature folder patterns. Do not reshape the project into a generic structure unless the task explicitly requires that migration.
...
- Keep shared components generic. Do not move feature-specific behavior into shared UI unless it is truly cross-cutting.
...
- If the app depends on an organization-specific UI or auth library, document that dependency in the app's `AGENTS.md` / Domain Skill — do not assume a shared components skill ships with Nero.
...
- Do not invent business rules or copy. If behavior, permission, validation, or microcopy is unclear, mark the assumption explicitly.
- Do not add packages without explicit technical and operational justification.
...
- Keep server/client boundaries explicit in Next.js. Do not move browser-only code into server components or server-only code into client bundles.
...
- Do not hide backend errors behind generic UI unless the product already defines that behavior.
...
Do not weaken tests to make failures pass. If behavior changed intentionally, update tests to the explicit new contract and record the affected user flow.
